...
model motorTorqueDemandElectricalDCBusprotect
parameter Modelica.Units.SI.Efficiency motorEfficiency = 0.95;

parameter Modelica.Units.SI.Voltage V_min  = 20;
parameter Modelica.Units.SI.Voltage V_soft  = 15;
Real vFactor;
...
equation
  ...
  i_cmd = vFactor * motorPowerActual / max(abs(voltageSensor.v), 1e-3);

  der(signalCurrent.i) = (i_cmd - signalCurrent.i) / Te;
  
  
vFactor =
  if voltageSensor.v > V_soft then
    1.0
  elseif voltageSensor.v > V_min then
    (voltageSensor.v - V_min) / (V_soft - V_min)
  else
    0.0;
...
end motorTorqueDemandElectricalDCBusprotect;
